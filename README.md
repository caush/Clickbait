...
Description of the challenge can be found in the [Webis.de](https://webis.de/events/clickbait-challenge/index.html) website. There is also a [Github page for the challenge](https://github.com/clickbait-challenge). Here is a synopsis of the challenge from the website: 
	...
	The task of the challenge is to develop an algorithm that rates how click baiting a social media post is.
...
The posts in the training and test sets have been judged on a 4-point scale (0, 0.3, 0.66, 1) by at least five annotators, and the structure of the truth.jsonl files is the following:
...
The Clickbait 2017 challenge corpus comprises a total of 38,517 Twitter posts annotated on a 4-point scale [not click baiting (0.0), slightly click baiting (0.33), considerably click baiting (0.66), heavily click baiting (1.0)] by five annotators from Amazon Mechanical Turk.
...
This set was not available for the challenge.

We extract the [instances.jsonl](Data/Original/clickbait17-train-170331/instances.jsonl) file from the "clickbait17-train-170331.zip" file. There are 2459 entries. We do not keep the media files.

Note that some of the entries do not have postText. We will remove them for training.

We extract the [truth.jsonl](Data/Original/clickbait17-train-170331/truth.jsonl) file from the "clickbait17-train-170331.zip" file. There are 2459 entries. From these entries, XXXX are clickbait (`truthClass == "clickbait"` or `truthMean > 0.5`).
...
### CuratedToModel
The [CuratedToModel.ipynb](CuratedToModel.ipynb) notebook takes the curated dataset to train different models.

### Caush
Models are saved locally in the "Caush" folder (not in Github) and in an [Huggingface folder](https://huggingface.co/caush).
